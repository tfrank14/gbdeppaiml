...
prepare_spec_object_ind <- function(loc, i, start.year = 1970, stop.year = 2019, popadjust = TRUE){
...
# create_hivproj_param <- function(loc, i, start.year = 1970, stop.year = 2019){
#   yr_start <- start.year
#   yr_end <- stop.year
...
#   
#   hivpop <- spec[run_num == i,.(sex, age, year, value = (pop_lt200 + pop_200to350 + pop_gt350 + pop_art))]
...
#   hivpop <- temp
#   
#   hivdeaths <- spec[run_num == i,.(sex, age, year, value = hiv_deaths)]
...
#   age14hivpop <- spec[run_num == i & age == 10, .(sex, year, pop_noart = (pop_lt200 + pop_200to350 + pop_gt350)/5, pop_art = pop_art/5)]
